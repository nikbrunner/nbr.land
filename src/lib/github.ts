...
export const getGithubPushEvents = async (username: string = "nikbrunner", limit: number = 25) => {
    try {
        ...
        const uniqueEvents = pushEvents.filter(
            (event, index, self) =>
                index === self.findIndex(e => e.payload.commits[0].message === event.payload.commits[0].message)
        );
